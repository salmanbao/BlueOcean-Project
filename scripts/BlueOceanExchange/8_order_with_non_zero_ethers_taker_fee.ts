...
async function main() {
    ...
    const buy = makeOrder(exchangeInstance.address, false, proxy, await signers[6].getAddress());
    const sell = makeOrder(exchangeInstance.address, true, proxy, await signers[7].getAddress());
    sell.side = 1
    buy.feeMethod = 1
    sell.feeMethod = 1
    ...
    sell.takerProtocolFee = 100 // 1%
    sell.takerRelayerFee = 100  // 1%
    buy.takerProtocolFee = 100  // 1%
    buy.takerRelayerFee = 100   // 1%
    buy.salt = 5
    sell.salt = 5
    ...
    await matchOrder(buy, sell, 0.0000001, identities, exchangeInstance)
}
